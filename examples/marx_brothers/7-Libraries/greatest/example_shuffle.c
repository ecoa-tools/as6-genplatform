...
#define COUNT_RUN(X)                                                   \
      ...
      SHUFFLE_TESTS (seed,
                     ...
                     COUNT_RUN (2);
                     COUNT_RUN (3);
                     COUNT_RUN (4); COUNT_RUN (5); COUNT_RUN (6); COUNT_RUN (7); COUNT_RUN (8); COUNT_RUN (9); COUNT_RUN (10);});
#undef COUNT_RUN
  ...
  SHUFFLE_TESTS (seed_of_time (),
                 ...
                 for (i = 0; i < limit; i++)
                 {
                 set_suffix (i); RUN_TEST1 (print_check_runs_and_pass, i);}
                 }
  );
...
SUITE (suite_shuffle_pass_and_failure)
{
  SHUFFLE_TESTS (seed_of_time (),
                 {
                 set_suffix (1);
                 RUN_TEST1 (just_print_and_pass, 1); set_suffix (2);
                 RUN_TEST1 (just_print_and_pass, 2); set_suffix (3);
                 RUN_TEST1 (just_print_and_pass, 3); set_suffix (4);
                 RUN_TEST1 (just_print_and_pass, 4); set_suffix (5);
                 RUN_TEST1 (just_print_and_pass, 5); RUN_TEST (just_fail);});
}
...
main (int argc, char **argv)
{
  ...
  SHUFFLE_SUITES (seed_of_time (),
                  ...
                  RUN_SUITE (suite2);
                  RUN_SUITE (suite3);
                  RUN_SUITE (suite4); RUN_SUITE (suite5); RUN_SUITE (suite_shuffle_pass_and_failure); RUN_SUITE (suite_failure);}
  );
