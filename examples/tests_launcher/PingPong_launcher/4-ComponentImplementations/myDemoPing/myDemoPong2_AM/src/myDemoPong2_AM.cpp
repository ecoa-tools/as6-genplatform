...
void Module::RR_msg_recv__request_received(const ECOA::uint32 ID, const mylib::t1 nb_send, const mylib::coord& coord_send){
	assert(nb_send == 118 and coord_send.x == 1516 and coord_send.y == 151617);
	mylib::coord coord_send_response = coord_send;
	mylib::t1 nb_send_response = nb_send + 1;
	container->RR_msg_recv__response_send(ID, nb_send_response, coord_send_response);
}
